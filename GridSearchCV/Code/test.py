...
class MyDataset(Dataset):
    def __init__(self, csv_dir, standard_csv_dir='./GridSearchCV/data/train_public.csv', mode='test'):
...
# 加载模型
model = joblib.load('./GridSearchCV/model/GridSearchCV_model.joblib')

# 初始化测试数据集
test_dataset = MyDataset('./GridSearchCV/data/test_public.csv')
...
# 保存结果到CSV文件
pd.DataFrame(results, columns=['id', 'isDefault']).to_csv('./GridSearchCV/output/submission.csv', index=False)
